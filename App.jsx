...
export default function KidQuest({ userId=null, userEmail=null, initialProfile=null, onSignOut=null }) {
  ...
  const [screen,    setScreen]    = useState(()=>{
    ...
    if(userId) return "loading"; // logged in but profile not loaded yet
  ...
  useEffect(()=>{
    ...
    // Sync navigation from profile (handles delayed load)
  ...
  if(screen==="loading") return (
    <Shell C={C}>
      ...
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:C.bg}}>
        <div style={{fontSize:64,marginBottom:16,animation:"float 1.5s ease-in-out infinite"}}>🦎</div>
        <div style={{fontWeight:900,fontSize:24,color:C.text,marginBottom:8}}>KidQuest</div>
        <div style={{width:40,height:40,borderRadius:"50%",border:`4px solid ${C.border}`,borderTopColor:C.mint,animation:"spin 0.8s linear infinite"}}/>
        <div style={{fontSize:13,color:C.textMed,marginTop:16}}>Cargando tu perfil…</div>
      </div>
    </Shell>
  );
