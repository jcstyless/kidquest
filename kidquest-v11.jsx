...
export default function KidQuest({ userId=null, userEmail=null, initialProfile=null, onSignOut=null }) {
  ...
  // navigation — initialize directly from initialProfile if available
  const [screen,    setScreen]    = useState(()=> initialProfile?.role ? "app" : "splash");
  const [role,      setRole]      = useState(()=> initialProfile?.role || null);
  const [tab,       setTab]       = useState(()=>{
    if(!initialProfile?.role) return "home";
    return initialProfile.role==="student"?"home":initialProfile.role==="parent"?"validate":"panel";
  });
  ...
  // ── SYNC AGE GROUP + CLEAR STALE DEMO DATA ──
  useEffect(()=>{
    if(initialProfile) {
      // Clear any stale demo localStorage data from previous sessions
      if(userId) {
        try {
          localStorage.removeItem("kq_v1_user");
          localStorage.removeItem("kq_v1_tasks");
        } catch(e){}
      }
  ...
  const submitTask = (partial=false) => {
    ...
    setUser(p=>({...p,xp:p.xp+bonusPts,coins:p.coins+bonusCoins,trophies:p.trophies+Math.floor(bonusPts/10)}));
  ...
  const spinWheel = () => {
    ...
    setTimeout(()=>{
      ...
      if(r.type==="coins")  setUser(p=>({...p,coins:p.coins+r.val}));
      if(r.type==="gems")   setUser(p=>({...p,gems:p.gems+r.val}));
      if(r.type==="xp")     setUser(p=>({...p,xp:p.xp+r.val}));
      if(r.type==="shield") setUser(p=>({...p,streakShields:p.streakShields+1}));
    },3800);
  };
    ...
    <Shell C={C}>
          ...
          <div className="modal pop-in">
            ...
            <BtnMain onClick={()=>{
              ...
              if(spendCat==="need") notify("Gasto necesario registrado ✓","🏠");
                ...
                setUser(p=>({...p,savingsGoal:{...p.savingsGoal,saved:p.savingsGoal.saved+Number(spendAmt)}}));
        ...
        <div className="overlay" style={{zIndex:10000}}>
          <div className="modal login-pop" style={{textAlign:"center",background:`linear-gradient(160deg,${C.card} 60%,${C.mintLt})`,border:`2px solid ${C.mint}50`}}>
            ...
            <div style={{position:"relative"}}>
              ...
              <BtnMain onClick={()=>{
                setUser(p=>({...p,gems:p.gems+loginBonusAmt}));
      ...
      <div style={{overflowY:"auto",maxHeight:"calc(100vh - 130px)",paddingBottom:85,background:C.bg}}>
            ...
            {[
              ...
              <div key={v.id} style={{background:C.card,borderRadius:18,padding:16,marginBottom:12,boxShadow:C.shadow}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>notify("Tarea rechazada — Mateo puede apelar","❌")} style={{flex:1,padding:"10px",borderRadius:12,border:`1.5px solid ${C.coral}60`,background:C.coralLt,color:C.coral,cursor:"pointer",fontWeight:800,fontSize:12}}>❌ Rechazar</button>
                  <button onClick={()=>{boom();notify("¡Aprobada! Mateo ganó sus recompensas 🎉","✅");}} style={{flex:2,padding:"10px",borderRadius:12,border:"none",background:`linear-gradient(135deg,${C.mint},${C.mintDk})`,color:"white",cursor:"pointer",fontWeight:800,fontSize:13}}>✅ Aprobar</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {role===ROLES.PARENT&&tab==="progress"&&(
          <div style={{padding:"16px 14px 0"}}>
            <div style={{fontWeight:800,fontSize:20,color:C.text,marginBottom:14}}>Progreso de Mateo</div>
            ...
            <div style={{background:C.card,borderRadius:16,padding:14,marginTop:12,marginBottom:12,boxShadow:C.shadow}}>
              ...
              <div style={{background:C.mintLt,borderRadius:10,padding:"8px 12px",fontSize:11,color:C.mintDk}}>
                💡 Pregúntale a Mateo: "¿En qué mes ahorraste más y por qué?"
              </div>
            </div>
        ...
        {role===ROLES.PARENT&&tab==="allowance"&&(
          <div style={{padding:"16px 14px 0"}}>
            <div style={{fontWeight:800,fontSize:20,color:C.text,marginBottom:4}}>Mesada de Mateo</div>
            ...
            <div style={{background:C.mintLt,borderRadius:14,padding:14,fontSize:13,color:C.mintDk,fontWeight:600,lineHeight:1.5}}>
              💡 <b>Tip para conversar con Mateo:</b> "Si ahorras $1.000 de tu mesada cada mes, en 1 año tendrás $12.000. ¿Qué te gustaría comprar con eso?"
            </div>
          </div>
        )}
        ...
        {role===ROLES.PARENT&&tab==="clanchat"&&(
          <ChatView msgs={aMsgs} setMsgs={setAMsgs} isMe={m=>m.author==="Mamá Laura"} myAuthor="Mamá Laura" myAvatar="👩" myRole="parent"
            input={chatInput} setInput={setChatInput} chatEndRef={chatEndRef} C={C}
        ...
        {role===ROLES.PARENT&&tab==="qrcode"&&(
          <div style={{padding:"16px 14px 0",textAlign:"center"}}>
            <div style={{fontWeight:800,fontSize:20,color:C.text,marginBottom:4}}>Mi código QR</div>
            <div style={{fontSize:13,color:C.textMed,marginBottom:24}}>Tu hijo te lo muestra cuando completa una tarea</div>
            <div style={{background:"white",borderRadius:24,padding:28,width:200,margin:"0 auto 14px",boxShadow:C.shadowMd,border:`2px solid ${C.gold}40`}}>
              <div style={{fontSize:80}}>🔲</div>
              <div style={{fontSize:11,color:C.textMed,fontWeight:700,marginTop:8}}>TUTOR-{Math.random().toString(36).slice(2,6).toUpperCase()}</div>
            </div>
            <div style={{fontSize:12,color:C.textLt,marginBottom:20}}>⏱ Válido 24 horas · se renueva automáticamente</div>
            <div style={{background:C.mintLt,borderRadius:16,padding:14,textAlign:"left"}}>
              {["Tu hijo toma foto o describe la tarea","La IA analiza la evidencia (flexible con luz)","Si hay duda, tu hijo te muestra el QR","Tú escaneas y la tarea queda doble validada","Si hay apelación, la IA la revisa y decide"].map((s,i)=>(
                <div key={i} style={{display:"flex",gap:8,fontSize:12,color:C.textMed,marginBottom:5}}>
                  <span style={{color:C.mint,fontWeight:800}}>{i+1}.</span>{s}
                </div>
              ))}
            </div>
          </div>
        ...
        {role===ROLES.TEACHER&&tab==="tchat"&&(
          <ChatView msgs={aMsgs} setMsgs={setAMsgs} isMe={m=>m.author==="Prof. García"} myAuthor="Prof. García" myAvatar="👨‍🏫" myRole="teacher"
            input={chatInput} setInput={setChatInput} chatEndRef={chatEndRef} C={C}
            ...
            onQuick={txt=>{setAMsgs(p=>[...p,{id:Date.now(),author:"Prof. García",avatar:"👨‍🏫",role:"teacher",text:txt.split(" ").slice(1).join(" ")||txt,time:now_t(),system:false}]);}}
            locked={false}/>
        )}
      </div>
